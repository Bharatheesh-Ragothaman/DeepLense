...
Implements the `Attention`, `PatchEmbed`, and `Mlp` classes
without relying on `timm`.
"""
...
class NanoDiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for i in range(depth)])
